---
description: "タスクを分解し、各サブタスクに Haiku/Sonnet の難易度を割り当てて計画ファイルを生成し、適切なモデルで実装する。"
version: "1.0.0"
context: fork
...
コードベースを調査し、タスクを細分化し、各サブタスクに難易度（Haiku / Sonnet）を割り当てて計画ファイルを生成する。実装フェーズでは適切なモデルで各タスクを実行する。

`$ARGUMENTS` が空の場合は、直近の会話コンテキストからタスクを読み取る。
...
**Explore エージェントは使わない。** ygrep skill を使ってコードベースを調査する。

```bash
ygrep "検索キーワード"          # ファイル・関数・クラスの検索
ygrep "query" -e ts -e tsx      # TypeScript ファイルに絞る
ygrep "query" -p src/           # パス絞り込み
ygrep "fn\\s+main" -r           # 正規表現検索
```

ygrep で結果が出ない場合のみ Grep / Glob にフォールバックする。
